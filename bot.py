...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await message.reply_text(
        ...
        "/export, /catalog, /dump – send full catalog export as a zipped CSV, "
...
async def export_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    The bot does NOT generate the catalog; it only sends a cached file
    generated by an external script (e.g. cron job).

    If the cache file is missing, it informs the user.
    """
    message = update.effective_message
    if message is None:
        return
    ...
    if not os.path.isfile(CATALOG_ZIP_PATH):
        await message.reply_text(
            ...
            "It will appear once the background job generates it."
        )
        return

    # Optionally, include some basic info (mtime, size)
    try:
        st = os.stat(CATALOG_ZIP_PATH)
        size_mb = st.st_size / (1024 * 1024)
        mtime = time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime(st.st_mtime))
        caption = (
            f"Catalog export (cached).\n"
    ...
    try:
        ...
        logger.error("Failed to send catalog zip: %s", e)
        await message.reply_text("Failed to send catalog export file.")
